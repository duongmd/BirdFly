...
class ViewController: UIViewController {
    ...
    func flyUpAndDown(){
        UIView.animateWithDuration(4,animations: {self.bird.center = CGPointMake(self.view.bounds.size.width, self.view.bounds.size.height)})
        {(finished) in
            ...
            UIView.animateWithDuration(3,animations: {self.bird.center = CGPointMake(0,self.view.bounds.size.height) })
            {(finished) in
            ...
            UIView.animateWithDuration(4,animations: {self.bird.center = CGPointMake(self.view.bounds.size.width, 0)})
            {(finished) in
                
            self.bird.transform = CGAffineTransformConcat(CGAffineTransformScale(self.bird.transform, -1, 1),CGAffineTransformMakeRotation(45))
                
            UIView.animateWithDuration(3,animations: {self.bird.center = CGPointMake(0, 0)})
            {(finished) in
